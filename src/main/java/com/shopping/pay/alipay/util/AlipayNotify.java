...
public class AlipayNotify {
	...
	public static Map<String, String> decrypt(AlipayConfig config, Map<String, String> inputPara) throws Exception {
		inputPara.put("notify_data", RSA.decrypt((String) inputPara.get("notify_data"), AlipayConfig.private_key,
	 ...
	 * 
	 * String partner = AlipayConfig.partner; String veryfy_url =
	 * HTTPS_VERIFY_URL + "partner=" + partner + "&notify_id=" + notify_id;
	 * 
	 * return checkUrl(veryfy_url); }
	 */
	...
	/*
	 * private static String checkUrl(String urlvalue) { String inputLine = "";
	 * try { URL url = new URL(urlvalue); HttpURLConnection urlConnection =
	 * (HttpURLConnection)url.openConnection(); BufferedReader in = new
	 * BufferedReader( new InputStreamReader(urlConnection.getInputStream()));
	 * inputLine = in.readLine().toString(); } catch (Exception e) {
	 * e.printStackTrace(); inputLine = ""; }
	 * 
	 * return inputLine; }
	 */
