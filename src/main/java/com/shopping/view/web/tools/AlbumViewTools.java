...
public class AlbumViewTools {
	...
	public List<Accessory> query_album(String id) {
		...
		if ((id != null) && (!id.equals(""))) {
			...
			list = this.accessoryService.query("select obj from Accessory obj where obj.album.id=:album_id", params, -1,
					-1);
		} else {
			list = this.accessoryService.query("select obj from Accessory obj where obj.album.id is null", null, -1,
					-1);
		}
		return list;
	}
