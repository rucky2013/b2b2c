...
public class SearchViewAction {
	...
	public ModelAndView search(HttpServletRequest request, HttpServletResponse response, String type, String keyword,
			...
			String view_type, String sc_id, String storeGrade_id, String checkbox_id, String storepoint, String area_id,
		...
		if ((shopping_view_type != null) && (!shopping_view_type.equals("")) && (shopping_view_type.equals("wap"))) {
			mv = new JModelAndView("wap/search.html", this.configService.getSysConfig(),
					this.userConfigService.getUserConfig(), 1, request, response);
		}
		...
		if (type.equals("store")) {
			...
			if ((shopping_view_type != null) && (!shopping_view_type.equals(""))
				...
				mv = new JModelAndView("wap/store_list.html", this.configService.getSysConfig(),
						this.userConfigService.getUserConfig(), 1, request, response);
			}
			...
			List areas = this.areaService
					.query("select obj from Area obj where obj.common =:common order by sequence asc", map, -1, -1);
	...
	public void searchAjax(HttpServletRequest request, HttpServletResponse response, String keyword, String currentPage,
			String orderBy, String orderType, String store_price_begin, String store_price_end, String view_type,
			String sc_id, String storeGrade_id, String checkbox_id, String storepoint) {
		...
		/*
		 * if (this.configService.getSysConfig().isZtc_status()) { Object
		 * ztc_map = new HashMap(); ((Map)ztc_map).put("ztc_status",
		 * Integer.valueOf(3)); ((Map)ztc_map).put("now_date", new Date());
		 * ((Map)ztc_map).put("ztc_gold", Integer.valueOf(0)); List ztc_goods =
		 * this.goodsService.query(
		 * "select obj from Goods obj where obj.ztc_status =:ztc_status and obj.ztc_begin_time <=:now_date and obj.ztc_gold>:ztc_gold order by obj.ztc_dredge_price desc"
		 * , (Map)ztc_map, 0, 5); mv.addObject("ztc_goods", ztc_goods); }
		 */
