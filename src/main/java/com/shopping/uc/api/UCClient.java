...
public class UCClient extends PHPFunctions {
	...
	public String uc_authcode(String string, String operation, String key, int expiry) {
		...
		if (operation.equals("DECODE")) {
			...
			if ((CommUtil.null2Int(substr(result.toString(), 0, 10)) == 0)
					|| (CommUtil.null2Long(substr(result.toString(), 0, 10)).longValue() - time() > 0L)) {
				if (substr(result.toString(), 10, 16).equals(substr(md5(substr(result.toString(), 26) + keyb), 0, 16)))
	...
	protected String uc_fopen(String url, int limit, String post, String cookie, boolean bysocket, String ip,
		...
		try {
			...
			path = matches.getPath() != null
					? matches.getPath() + (matches.getQuery() != null ? "?" + matches.getQuery() : "") : "/";
