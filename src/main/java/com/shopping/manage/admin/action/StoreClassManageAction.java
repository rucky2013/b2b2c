...
public class StoreClassManageAction {
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		ModelAndView mv = new JModelAndView("admin/blue/storeclass_list.html", this.configService.getSysConfig(),
		...
		String params = "";
		StoreClassQueryObject qo = new StoreClassQueryObject(currentPage, mv, orderBy, orderType);
		qo.setOrderBy("sequence");
		qo.setOrderType("asc");
		qo.addQuery("obj.parent is null", null);
		IPageList pList = this.storeclassService.list(qo);
		CommUtil.saveIPageList2ModelAndView(url + "/admin/storeclass_list.htm", "", params, pList, mv);
		return mv;
	}
	...
	public ModelAndView add(HttpServletRequest request, HttpServletResponse response, String pid) {
		ModelAndView mv = new JModelAndView("admin/blue/storeclass_add.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		List scs = this.storeclassService.query(
				"select obj from StoreClass obj where obj.parent is null order by obj.sequence asc", null, -1, -1);
		mv.addObject("scs", scs);
		if ((pid != null) && (!pid.equals(""))) {
			StoreClass obj = new StoreClass();
			obj.setParent(this.storeclassService.getObjById(Long.valueOf(Long.parseLong(pid))));
	...
	public ModelAndView edit(HttpServletRequest request, HttpServletResponse response, String id, String currentPage) {
		...
		if ((id != null) && (!id.equals(""))) {
			StoreClass storeclass = this.storeclassService.getObjById(Long.valueOf(Long.parseLong(id)));
			List scs = this.storeclassService.query(
					"select obj from StoreClass obj where obj.parent is null order by obj.sequence asc", null, -1, -1);
			mv.addObject("scs", scs);
	...
	public ModelAndView save(HttpServletRequest request, HttpServletResponse response, String id, String pid,
		...
		if ((pid != null) && (!pid.equals(""))) {
			StoreClass parent = this.storeclassService.getObjById(Long.valueOf(Long.parseLong(pid)));
		...
		for (String id : ids) {
			if (!id.equals("")) {
				...
				List<StoreClass> scs = this.storeclassService.query(
						"select obj from StoreClass obj where obj.id in (:ids) order by obj.level desc", params, -1,
						-1);
	...
	public void storeclass_verify(HttpServletRequest request, HttpServletResponse response, String className,
			String id) {
		boolean ret = true;
		Map params = new HashMap();
		...
		List gcs = this.storeclassService
				.query("select obj from StoreClass obj where obj.className=:className and obj.id!=:id", params, -1, -1);
	...
	public ModelAndView storeclass_data(HttpServletRequest request, HttpServletResponse response, String pid,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/storeclass_data.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Map map = new HashMap();
		map.put("pid", Long.valueOf(Long.parseLong(pid)));
		List gcs = this.storeclassService.query("select obj from StoreClass obj where obj.parent.id =:pid", map, -1,
				-1);
