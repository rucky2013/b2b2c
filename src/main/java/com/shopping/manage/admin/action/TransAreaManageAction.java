...
public class TransAreaManageAction {
	...
	public ModelAndView trans_area_list(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		ModelAndView mv = new JModelAndView("admin/blue/trans_area_list.html", this.configService.getSysConfig(),
		...
		if ((pid == null) || (pid.equals(""))) {
			...
			params = "&pid=" + pid;
			TransArea parent = this.transareaService.getObjById(Long.valueOf(Long.parseLong(pid)));
			mv.addObject("parent", parent);
			if (parent.getLevel() == 0) {
				Map map = new HashMap();
				map.put("pid", parent.getId());
				List seconds = this.transareaService.query("select obj from TransArea obj where obj.parent.id=:pid",
						map, -1, -1);
			...
			if (parent.getLevel() == 1) {
				Map map = new HashMap();
				map.put("pid", parent.getId());
				List thirds = this.transareaService.query("select obj from TransArea obj where obj.parent.id=:pid", map,
						-1, -1);
				map.clear();
		...
		CommUtil.saveIPageList2ModelAndView(url + "/admin/trans_area_list.htm", "", params, pList, mv);
		List areas = this.transareaService.query("select obj from TransArea obj where obj.parent.id is null", null, -1,
				-1);
	...
	public ModelAndView trans_area_save(HttpServletRequest request, HttpServletResponse response, String areaId,
		...
		if (areaId != null) {
			...
			for (String id : ids) {
				...
				TransArea area = this.transareaService
						.getObjById(Long.valueOf(Long.parseLong(request.getParameter("id_" + i))));
	...
	public String trans_area_del(HttpServletRequest request, String mulitId, String currentPage, String pid) {
		...
		for (String id : ids) {
			if (!id.equals("")) {
				Set list = genericIds(this.transareaService.getObjById(Long.valueOf(Long.parseLong(id))));
				Map params = new HashMap();
				params.put("ids", list);
				List<TransArea> objs = this.transareaService
						.query("select obj from TransArea obj where obj.id in (:ids)", params, -1, -1);
