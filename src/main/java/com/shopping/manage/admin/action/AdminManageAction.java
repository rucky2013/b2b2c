...
public class AdminManageAction implements ServletContextAware {
	...
	public ModelAndView admin_list(String currentPage, String orderBy, String orderType, HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView("admin/blue/admin_list.html", this.configService.getSysConfig(),
	...
	public ModelAndView admin_add(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/admin_add.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView admin_save(HttpServletRequest request, HttpServletResponse response, String id, String role_ids,
			String list_url, String add_url, String password) {
		/*
		 * WebForm wf = new WebForm(); User user = null; if (id.equals("")) {
		 * user = (User) wf.toPo(request, User.class);
		 * user.setPassword(Md5Encrypt.md5(password).toLowerCase());
		 * user.setAddTime(new Date()); } else { User u =
		 * this.userService.getObjById(Long.valueOf(Long.parseLong(id))); user =
		 * (User) wf.toPo(request, u); } user.getRoles().clear(); if
		 * (user.getUserRole().equalsIgnoreCase("ADMIN")) { Map params = new
		 * HashMap(); params.put("display", Boolean.valueOf(false));
		 * params.put("type", "ADMIN"); params.put("type1", "BUYER"); List roles
		 * = this.roleService.query(
		 * "select obj from Role obj where (obj.display=:display and obj.type=:type) or obj.type=:type1"
		 * , params, -1, -1); user.getRoles().addAll(roles); } String[] rids =
		 * role_ids.split(",");
		 * 
		 * for (String rid : rids) { if (!rid.equals("")) { Role role =
		 * this.roleService.getObjById(Long.valueOf(Long.parseLong(rid)));
		 * user.getRoles().add(role); } } if (id.equals("")) {
		 * this.userService.save(user); } else { this.userService.update(user);
		 * }
		 * 
		 * ModelAndView mv = new JModelAndView("admin/blue/success.html",
		 * this.configService.getSysConfig(),
		 * this.userConfigService.getUserConfig(), 0, request, response);
		 * mv.addObject("list_url", list_url); mv.addObject("op_title",
		 * "保存管理员成功"); if (add_url != null) { mv.addObject("add_url", add_url);
		 * } return mv;
		 */
		WebForm wf = new WebForm();
		User user = null;
	...
	public ModelAndView admin_pws(HttpServletRequest request, HttpServletResponse response) {
		ModelAndView mv = new JModelAndView("admin/blue/admin_pws.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView admin_pws_save(HttpServletRequest request, HttpServletResponse response, String old_password,
			String password) {
		ModelAndView mv = new JModelAndView("admin/blue/success.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public String init_role() {
		...
		if ((current_user != null) && (current_user.getUserRole().indexOf("ADMIN") >= 0)
					...
					for (Method m : ms) {
						...
						for (Annotation tag : annotation) {
							if (SecurityMapping.class.isAssignableFrom(tag.annotationType())) {
								...
								List ress = this.resService.query("select obj from Res obj where obj.value=:value",
										params, -1, -1);
								if (ress.size() == 0) {
									Res res = new Res();
									...
									List roles = this.roleService.query(
											"select obj from Role obj where obj.roleCode=:roleCode", params, -1, -1);
									Role role = null;
									if (roles.size() > 0) {
			...
			List<Role> roles = this.roleService.query("select obj from Role obj order by obj.addTime desc", null, -1,
					-1);
			if (user == null) {
				user = new User();
			...
			List admin_roles = this.roleService.query(
					"select obj from Role obj where obj.display=:display and obj.type=:type", (Map) params, -1, -1);
			((Map) params).clear();
			((Map) params).put("type", "BUYER");
			List buyer_roles = this.roleService.query("select obj from Role obj where obj.type=:type", (Map) params, -1,
					-1);
			((Map) params).clear();
			((Map) params).put("userRole", "ADMIN");
			((Map) params).put("userName", "admin");
			List<User> admins = this.userService.query(
					"select obj from User obj where obj.userRole=:userRole and obj.userName!=:userName", (Map) params,
					-1, -1);
			...
			List<User> buyers = this.userService.query("select obj from User obj where obj.userRole=:userRole",
					(Map) params, -1, -1);
			...
			List seller_roles = this.roleService
					.query("select obj from Role obj where (obj.type=:type1 or obj.type=:type2)", (Map) params, -1, -1);
			((Map) params).clear();
			((Map) params).put("userRole1", "BUYER_SELLER");
			((Map) params).put("userRole2", "ADMIN_BUYER_SELLER");
			((Map) params).put("userRole3", "ADMIN");
			((Map) params).put("userName", "admin");
			List<User> sellers = this.userService.query(
