...
public class SpareGoodsManageAction {
	...
	public ModelAndView spare_goods(HttpServletRequest request, HttpServletResponse response, String currentPage,
		...
		ModelAndView mv = new JModelAndView("admin/blue/sparegoods_list.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		SpareGoodsQueryObject qo = new SpareGoodsQueryObject(currentPage, mv, orderBy, orderType);
		if ((type != null) && (!type.equals(""))) {
			qo.addQuery("obj.status", new SysMap("obj_status", Integer.valueOf(CommUtil.null2Int(type))), "=");
	...
	public String sparegoods_switch(HttpServletRequest request, HttpServletResponse response, String id,
		...
		return "redirect:sparegoods_list.htm?currentPage=" + currentPage + (type != null ? "&type=" + type : "");
	}
	...
	public String sparegoods_delete(HttpServletRequest request, HttpServletResponse response, String mulitId,
			String currentPage) {
	...
	public void sparegoods_ajax(HttpServletRequest request, HttpServletResponse response, String id, String fieldName,
	...
	public ModelAndView sparegoods_class_list(HttpServletRequest request, HttpServletResponse response,
			String currentPage, String orderBy, String orderType) {
		ModelAndView mv = new JModelAndView("admin/blue/sparegoods_class_list.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView sparegoodsclass_add(HttpServletRequest request, HttpServletResponse response, String pid) {
		ModelAndView mv = new JModelAndView("admin/blue/sparegoods_class_add.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		List sparegoodsClass = this.sparegoodsclassService
				.query("select obj from SpareGoodsClass obj where obj.parent.id is null", null, -1, -1);
		mv.addObject("sgcs", sparegoodsClass);
		if ((pid != null) && (!pid.equals(""))) {
			SpareGoodsClass parent = this.sparegoodsclassService.getObjById(CommUtil.null2Long(pid));
	...
	public ModelAndView sparegoods_class_edit(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage) {
		ModelAndView mv = new JModelAndView("admin/blue/sparegoods_class_add.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		if ((id != null) && (!id.equals(""))) {
			SpareGoodsClass sparegoodsclass = this.sparegoodsclassService.getObjById(Long.valueOf(Long.parseLong(id)));
	...
	public ModelAndView sparegoods_class_save(HttpServletRequest request, HttpServletResponse response, String id,
			String currentPage, String list_url, String add_url, String pid) {
		...
		if (add_url != null) {
			mv.addObject("add_url", add_url + "?pid=" + pid + "&currentPage=" + currentPage);
		}
		return mv;
	}
	...
	public String sparegoods_class_del(HttpServletRequest request, HttpServletResponse response, String mulitId,
			String currentPage) {
		String[] ids = mulitId.split(",");
		for (String id : ids) {
			if (!id.equals("")) {
				Set list = genericIds(this.sparegoodsclassService.getObjById(Long.valueOf(Long.parseLong(id))));
				Map params = new HashMap();
				params.put("ids", list);
				List<SpareGoodsClass> sgcs = this.sparegoodsclassService.query(
						"select obj from SpareGoodsClass obj where obj.id in (:ids) order by obj.level desc", params,
						-1, -1);
	...
	public void sparegoods_class_ajax(HttpServletRequest request, HttpServletResponse response, String id,
	...
	public ModelAndView sparegoods_class_data(HttpServletRequest request, HttpServletResponse response, String pid) {
		ModelAndView mv = new JModelAndView("admin/blue/sparegoods_class_data.html", this.configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Map map = new HashMap();
		map.put("pid", Long.valueOf(Long.parseLong(pid)));
		List gcs = this.sparegoodsclassService.query("select obj from SpareGoodsClass obj where obj.parent.id =:pid",
				map, -1, -1);
	...
	public void sparegoods_class_verify(HttpServletRequest request, HttpServletResponse response, String className,
			String pid) {
		boolean ret = true;
		List sgcs = null;
		Map params = new HashMap();
		...
		if (pid.equals("")) {
			sgcs = this.sparegoodsclassService
					.query("select obj from SpareGoodsClass obj where obj.className=:className", params, -1, -1);
		} else {
			params.put("pid", CommUtil.null2Long(pid));
			sgcs = this.sparegoodsclassService.query(
