...
public class OrderSellerAction {
	...
	public ModelAndView order_shipping(HttpServletRequest request, HttpServletResponse response, String id,
		...
		if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
			...
			List<GoodsCart> goodsCarts = this.goodsCartService
					.query("select obj from GoodsCart obj where obj.of.id = :oid", map, -1, -1);
			...
			List expressCompanys = this.expressCompayService.query(
		...
		if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
			...
			if (obj.getPayment().getMark().equals("alipay")) {
				...
				if ((payments.size() > 0) && (payments.get(0) != null)) {
					if (!CommUtil.null2String(((Payment) payments.get(0)).getSafeKey()).equals("")) {
						if (!CommUtil.null2String(((Payment) payments.get(0)).getPartner()).equals("")) {
	...
	public String order_refund_save(HttpServletRequest request, HttpServletResponse response, String id,
		...
		if (obj.getStore().getId().equals(SecurityUserHolder.getCurrentUser().getStore().getId())) {
			...
			if (type.equals(refund_type)) {
				...
				buyer.setAvailableBalance(BigDecimal.valueOf(
						CommUtil.add(buyer.getAvailableBalance(), BigDecimal.valueOf(CommUtil.null2Double(refund)))));
					...
					if (count > 0) {
						...
						if (obj.getPayment().getMark().equals("balance")) {
							...
							seller.setAvailableBalance(
									BigDecimal.valueOf(CommUtil.subtract(seller.getAvailableBalance(), balance)));
							this.userService.update(seller);
							User buyer = obj.getUser();
							buyer.setAvailableBalance(
									BigDecimal.valueOf(CommUtil.add(buyer.getAvailableBalance(), balance)));
							this.userService.update(buyer);
						}
					}
				}
				...
				while (enum1.hasMoreElements()) {
					...
					if (paramName.indexOf("evaluate_seller_val") >= 0) {
						...
						if (this.configService.getSysConfig().isIntegral()) {
							...
							integral = integral > this.configService.getSysConfig().getEveryIndentLimit()
									? this.configService.getSysConfig().getEveryIndentLimit() : integral;
	...
	private TransInfo query_ship_getData(String id) {
		...
		try {
			URL url = new URL("http://api.kuaidi100.com/api?id=" + this.configService.getSysConfig().getKuaidi_id()
					+ "&com=" + (obj.getEc() != null ? obj.getEc().getCompany_mark() : "") + "&nu=" + obj.getShipCode()
			...
			/*
			 * byte[] b = new byte[10000]; int numRead = urlStream.read(b);
			 * String content = new String(b, 0, numRead, charSet); while
			 * (numRead != -1) { numRead = urlStream.read(b); if (numRead == -1)
			 * continue; String newContent = new String(b, 0, numRead, charSet);
	...
	public String seller_order_return(HttpServletRequest request, HttpServletResponse response, String id, String gr_id,
	...
	public ModelAndView seller_order_return_confirm(HttpServletRequest request, HttpServletResponse response,
			String id) {
	...
	private void send_email(HttpServletRequest request, OrderForm order, String mark) throws Exception {
		...
		if ((template != null) && (template.isOpen())) {
			...
			p.setProperty("file.resource.loader.path",
					request.getRealPath("") + File.separator + "vm" + File.separator);
