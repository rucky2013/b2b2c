			...
			for (GoodsCart gc : sc.getGcs()) {
				...
				if (goods.getGoods_transfee() == 0) {
					if (goods.getTransport() != null) {
						mail_fee = mail_fee + cal_order_trans(goods.getTransport().getTrans_mail_info(),
								goods.getTransport().getTrans_type(), goods.getGoods_weight(), goods.getGoods_volume(),
								city_name);

						express_fee = express_fee + cal_order_trans(goods.getTransport().getTrans_express_info(),
								goods.getTransport().getTrans_type(), goods.getGoods_weight(), goods.getGoods_volume(),
								city_name);

						ems_fee = ems_fee + cal_order_trans(goods.getTransport().getTrans_ems_info(),
								goods.getTransport().getTrans_type(), goods.getGoods_weight(), goods.getGoods_volume(),
								city_name);
					} else {
			...
			for (Map map : list) {
				...
				for (String city : city_list) {
					if ((city.equals(city_name)) || (city_name.indexOf(city) == 0)) {
						...
						if ((trans_type == 1) && (CommUtil.null2Float(goods_weight) > 0.0F)) {
							fee = trans_fee;
							float other_price = 0.0F;
							if (trans_add_weight > 0.0F) {
								other_price = trans_add_fee
										* (float) Math.round(
												Math.ceil(CommUtil.subtract(goods_weight, Float.valueOf(trans_weight))))
										/ trans_add_fee;
							}
							fee += other_price;
						}
						...
						if (trans_add_weight > 0.0F) {
							other_price = trans_add_fee
									* (float) Math.round(
											Math.ceil(CommUtil.subtract(goods_volume, Float.valueOf(trans_weight))))
									/ trans_add_fee;
						}
						fee += other_price;
					...
					for (String city : city_list) {
						if (city.equals("全国")) {
							...
							if ((trans_type == 1) && (CommUtil.null2Float(goods_weight) > 0.0F)) {
								...
								if (trans_add_weight > 0.0F) {
									other_price = trans_add_fee
											* (float) Math.round(Math
													.ceil(CommUtil.subtract(goods_weight, Float.valueOf(trans_weight))))
											/ trans_add_fee;
								}
								fee += other_price;
							}
