...
public class WxCommonUtil {
	...
	public static WxToken getToken(String appid, String appsecret) {
		...
		if (null != jsonObject) {
			try {
				...
				WxToken = null;
				// 获取WxToken失败
				log.error("获取WxToken失败 errcode:{} errmsg:{}", jsonObject.getInt("errcode"),
	 ...
	 */
	/*
	 * public static AccessToken getAccessToken(SystemService
	 * systemService,String appid,String appsecret) {
	 * 
	 * AccessTokenYw accessTocken = getRealAccessToken(systemService);
	 * 
	 * if(accessTocken!=null){ SimpleDateFormat sdf = new SimpleDateFormat(
	 * "yyyy-MM-dd HH:mm:ss"); java.util.Date end = new java.util.Date();
	 * java.util.Date start = new
	 * java.util.Date(accessTocken.getAddTime().getTime());
	 * if(end.getTime()-accessTocken.getAddTime().getTime()>accessTocken.
	 * getExpires_in()*1000){ AccessToken accessToken = null; String requestUrl
	 * = access_token_url.replace("APPID", appid).replace("APPSECRET",
	 * appsecret); JSONObject jsonObject = httpRequest(requestUrl, "GET", null);
	 * // 如果请求成功 if (null != jsonObject) { try { accessToken = new
	 * AccessToken();
	 * accessToken.setToken(jsonObject.getString("access_token"));
	 * accessToken.setExpiresIn(jsonObject.getInt("expires_in")); //凭证过期更新凭证
	 ...
	 * updateAccessToken(atyw,systemService); } catch (Exception e) {
	 * accessToken = null; // 获取token失败 String wrongMessage =
	 * "获取token失败 errcode:{} errmsg:{}"
	 * +jsonObject.getInt("errcode")+jsonObject.getString("errmsg");
	 * log.info(wrongMessage); } } return accessToken; }else{
	 * 
	 * AccessToken accessToken = new AccessToken();
	 * accessToken.setToken(accessTocken.getAccess_token());
	 * accessToken.setExpiresIn(accessTocken.getExpires_in()); return
	 * accessToken; } }else{
	 * 
	 * AccessToken accessToken = null; String requestUrl =
	 ...
	 * if (null != jsonObject) { try { accessToken = new AccessToken();
	 * accessToken.setToken(jsonObject.getString("access_token"));
	 ...
	 * 
	 * } catch (Exception e) { accessToken = null; // 获取token失败 String
	 * wrongMessage = "获取token失败 errcode:{} errmsg:{}"
	 * +jsonObject.getInt("errcode")+jsonObject.getString("errmsg");
	 * log.info(wrongMessage); } } return accessToken; } }
	 */
	 ...
	 */
	/*
	 * public static AccessTokenYw getRealAccessToken(SystemService
	 * systemService){ List<AccessTokenYw> accessTockenList =
	 * systemService.findByQueryString("from AccessTokenYw"); return
	 * accessTockenList.get(0); }
	 */
	 ...
	 */
	/*
	 * public static void saveAccessToken( AccessTokenYw
	 * accessTocken,SystemService systemService){
	 ...
	 */
	/*
	 * public static void updateAccessToken( AccessTokenYw
	 * accessTocken,SystemService systemService){ String sql =
	 * "update accesstoken set access_token='"
	 * +accessTocken.getAccess_token()+"',expires_ib="+accessTocken.
	 * getExpires_in()+",addtime=now() where id='"+accessTocken.getId()+"'";
	 ...
	 */
	/*
	 * public static String decryptMsg(String msgSignature,String
	 * timeStamp,String nonce,String encrypt_msg) { WXBizMsgCrypt pc; String
	 * result =""; try { pc = new WXBizMsgCrypt(token, encodingAesKey, appId);
	 * result = pc.decryptMsg(msgSignature, timeStamp, nonce, encrypt_msg); }
	 * catch (AesException e) { // TODO Auto-generated catch block
	 ...
	 */
	/*
	 * public static String ecryptMsg(String replayMsg,String timeStamp, String
	 * nonce) { WXBizMsgCrypt pc; String result =""; try { pc = new
	 * WXBizMsgCrypt(token, encodingAesKey, appId); result =
	 * pc.encryptMsg(replayMsg, timeStamp, nonce); } catch (AesException e) { //
